...
/* q is a query constructed by QSqlQuery.prepare(QString& ) which contains placeholders
   placeholders could be ?        which can be filled by QSqlQuery::addBindValue(QVariant& )
                         :varname which can be filled by QSqlQuery::bindValue(":varname", var)
   but these two can't be both used in one query                                                */
...
QVariant addGenre(QSqlQuery& q, const QString& name)
{
    ...
    return q.lastInsertId();    // last insert row id if insert successfully. For example, MySQL's auto increment field.
}
...
/* ------- create tables if there isn't any ------- */
    ...
    if(!q.exec(QLatin1String("create table authors(id        integer primary key,"
                                                  "name      varchar,"
                                                  "birthdate date    )"))){ return q.lastError(); }
    if(!q.exec(QLatin1String("create table genres(id   integer primary key,"
                                                 "name varchar             )"))){ return q.lastError(); }
