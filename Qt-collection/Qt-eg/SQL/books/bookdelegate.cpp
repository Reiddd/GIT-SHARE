...
void BookDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    if(index.column() != 5){
        ...
        opt.rect.adjust(0, 0, -1, -1);                                                                                                                                              // QStyleOptionViewItem contains all parameters used to draw an item in a view widget
        QSqlRelationalDelegate::paint(painter, opt, index);
    }
    else{
        const QAbstractItemModel* model = index.model();                                                                                                                            // const QAbstractItemModel* QModelIndex::model()
        ...
        int rating = model->data(index, Qt::DisplayRole).toInt();                                                                                                                   // QVariant QAbstractItemModel::data(const QModelIndex& index, int role = Qt::DisplayRole)
